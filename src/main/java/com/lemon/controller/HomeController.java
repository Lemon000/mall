...
public class HomeController {
    ...
    public String showGoods(Model model, HttpServletRequest request, Map<String, Integer> map,HttpSession session) {
        String user = (String) session.getAttribute("LoginUser");
        String id = request.getParameter("id");
        System.out.println(id);
        GoodsEntity goodsEntity = goodsDao.selectGoodsById(id);
        model.addAttribute("detail", goodsEntity);
        int count = cartDao.SelectCount(user);
        map.put("cart_count", count);
        return "detail";
    }
    @RequestMapping("/xiangqing1")
    public String show_Goods(Model model, HttpServletRequest request, Map<String, Integer> map,HttpSession session) {
        String user = (String) session.getAttribute("LoginUser");
        String id = request.getParameter("id");
        System.out.println(id);
    ...
    @RequestMapping("/find")
    public String find(HttpServletRequest request,Model model,Map<String, Integer> map1,HttpSession session){
